...
class JsonParseError extends S.TaggedErrorClass<JsonParseError>()(
  ...
  S.annote("JsonParseError", { title: "JSON Parse Error", description: "Failed to parse JSON string" })
) {}
...
4. Every error class extends `TaggedErrorClass` from `effect/schema` with Identity-based key and annotations.
