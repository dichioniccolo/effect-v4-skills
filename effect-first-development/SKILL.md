...
8. For tooling source, use `TaggedErrorClass` from `effect/schema`.
...
class JsonParseError extends S.TaggedErrorClass<JsonParseError>()(
  "JsonParseError",
  { message: S.String, input: S.String },
  S.annote("JsonParseError", { description: "Invalid JSON payload" })
) {}
...
export class JobQueued extends S.Class<JobQueued>()(
  { state: S.tag("queued") },
  S.annote("JobQueued", { description: "Queued job state." })
) {}

export class JobRunning extends S.Class<JobRunning>()(
  { state: S.tag("running"), workerId: S.String },
  S.annote("JobRunning", { description: "Running job state." })
) {}

export class JobFailed extends S.Class<JobFailed>()(
  { state: S.tag("failed"), reason: S.String },
  S.annote("JobFailed", { description: "Failed job state." })
) {}
...
export const JobState = JobStateTag
  ...
  .annotate(S.annote("JobState", { description: "Job lifecycle state union." }))
...
export class ExternalTaskCreated extends S.Class<ExternalTaskCreated>()(
{
  kind: S.tag("created"),
  id: S.String
}) {}

export class ExternalTaskCompleted extends S.Class<ExternalTaskCompleted>()(
{
...
export const ExternalTaskEvent = ExternalTaskKind
  ...
  .annotate(S.annote("ExternalTaskEvent", {
...
const runOperation = Effect.fn("Operation.run")(function* (id: string) {
  ...
  Effect.annotateLogs({ service: "app-service" }),
...
export class ProfileInput extends S.Class<ProfileInput>()(
{
